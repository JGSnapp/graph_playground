...
export const arrowCreate: ToolSpec = {
  name: 'arrow_create',
  description:
    'Соединяет два артефакта стрелкой. Сторона присоединения задаётся явно (top/right/bottom/left) или auto — тогда сторона выбирается по взаимному расположению. Точку на стороне можно зафиксировать через fromOffset/toOffset (0.5 — центр); без них порты распределяются автоматически, чтобы параллельные стрелки не сливались. Если стрелка подходит к стороне плашмя (меньше 30°) или упирается во встречный конец другой стрелки (ближе 48px), тул чинит это сам — сдвигает порт, меняет сторону или добавляет перпендикулярный изгиб — и перечисляет правки в поле adjustments. Если крепление задано пользователем и менять его нельзя, передай exact=true: тогда тул сохранит всё как есть и только предупредит.',
