...
export const boardScreenshot: ToolSpec = {
  ...
  run: async (args, ctx) => {
    const region = resolveRegion(args, ctx);
    const dataUrl = await ctx.screenshots.request(ctx.boardId, region, ctx.emit);
    if (!dataUrl) {
      ...
      return {
        data: {
          region,
          captured: false,
          note: 'Клиент не ответил, возвращена ASCII-схема области.',
          schema,
        },
      };
    }
...
export const boardCheckIntersections: ToolSpec = {
  ...
  description:
    'Проверяет качество раскладки: стрелки, режущие артефакты (с какой стороны входят/выходят), пересечения стрелок с углом пересечения, стрелки, слипшиеся в одну линию, слишком малые зазоры, наложения артефактов и подписи, которым не хватает места. Возвращает метрику качества и сравнение с прошлой проверкой. Вызывай после расстановки и после каждой правки.',
  parameters: objectSchema({
    ...regionProps,
